function Get-SharePointTenantSiteIdByUrl
{
    ...
    process
    {
        ...
        {
            $endpoint = '/_api/web/lists/GetByTitle(''DO_NOT_DELETE_SPLIST_TENANTADMIN_AGGREGATED_SITECOLLECTIONS'')/items?$filter=SiteUrl eq ''{0}''&$select=SiteId,SiteCreationSource' -f $SiteUrl.ToString().Trim().TrimEnd("/")
        ...
        {
            return [Microsoft.SharePoint.Client.TenantExtensions]::GetSiteGuidByUrl( $tenant, $SiteUrl.ToString().Trim().TrimEnd("/") )
        }   
    }
    end
    {
